...
$DAgentInstall = .\DeployAgent.ps1 -AgentBootServiceInstallerFolder "$DeployAgentLocation\RDAgentBootLoaderInstall" `
    ...
    -FSLogixAppsInstallerFolder "$DeployAgentLocation\FSLogixAgentInstall" `
    -UserProfileTargetPath $UserProfileTargetPath `
    -WorkspaceID $WorkspaceID `
    -WorkspacePrimaryKey $WorkspacePrimaryKey `
    -RegistrationToken $Registered.Token `
    -StartAgent $true

Write-Log -Message "DeployAgent Script was successfully executed and RDAgentBootLoader, RDAgent, FSLogix Apps installed inside VM for existing hostpool: $HostPoolName`n$DAgentInstall"
